...
from .schemas import changePasswordRequest, updateRequestUser
...
class AuthServices:
                ...
                "results": [
                    {
                        ...
                        "midle_name": item.midle_name,
                        "last_name": item.last_name,
                        "mobile": item.mobile,
                        "email": item.email,
                        "registered_at": item.registered_at,
                        "last_login": item.last_login if item.last_login else None,
                        "intro": item.intro,
    ...
    async def get_detail(requestUser: User, username: str):
        ...
        return Ok(
            data={"username": data.username, "name": data.name, "email": data.email}
        )
    ...
    async def update_user(requestUser: User, request: updateRequestUser):
        ...
        if requestUser.username == request.username:
            ...
            data = authRepository.update_user(
                username=username, email=request.email, name=request.name
            )
            return Ok(
                data={"username": data.username, "name": data.name, "email": data.email}
            )
            ...
            data = authRepository.update_user_and_username(
                username=requestUser.username,
    ...
    async def update_password(requestUser: User, request: changePasswordRequest):
        ...
        authRepository.update_password(
            username=requestUser.username, new_password=password_hashed
        )
    ...
    async def sign_up(request):
        ...
        # create new user
        new_user = authRepository.create_new_user(
            username=username, name=name, password=password_hashed, email=email
        )

        return Created(data={"username": new_user.username, "email": new_user.email})
    ...
    async def generate_token(form_data: OAuth2PasswordRequestForm):
        try:
            ...
            if (
                authRepository.verify_password(
                    user=form_data.username, plain_password=form_data.password
                )
                != True
            ):
